...
import { ncrDistrictGroups } from './ncrData';
...
function OnboardModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const finish = async () => {
    ...
    const response = await fetch('/api/profile', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ created_for: createdFor, ministry: selectedMinistry, organisation: selectedOrganisation, service, residence_city: ncrBase, posting_outlook: postingOutlook, preferred_hubs: preferredHubs }) }).catch(() => null);
    setSaving(false);
