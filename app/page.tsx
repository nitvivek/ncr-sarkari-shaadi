...
const matches = [
  ...
  { id: 2, initials: 'RK', name: 'Rohan K.', gender: 'male', role: 'Assistant · Delhi Govt.', city: 'Noida', fit: 92, tone: 'sage', note: 'Aligned on NCR career plans' },
...
function AuthModal({ onClose, onEnter }: { onClose: () => void; onEnter: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <div className="modal auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title" onMouseDown={(e) => e.stopPropagation()}>
      ...
      {mode === 'signup' && <label className="field"><span>Full name</span><input placeholder="Your name" /></label>}
      <label className="field"><span>Mobile number</span><input type="tel" placeholder="+91 98••• •••••" /></label>
      <label className="field"><span>Password</span><input type="password" placeholder="At least 8 characters" /></label>
      <button className="button button--primary button--full" onClick={onEnter}>{mode === 'login' ? 'Continue securely' : 'Create profile'} <Icon name="arrow" size={16} /></button>
...
function OnboardModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <div className="modal onboard-modal" role="dialog" aria-modal="true" aria-labelledby="onboard-title" onMouseDown={(e) => e.stopPropagation()}>
      ...
      {step === 1 && <><h2 id="onboard-title">Start with what keeps you here</h2><p className="modal-copy">Your posting plan matters more than a pin on a map.</p><label className="field"><span>Profile created for</span><select defaultValue="Myself"><option>Myself</option><option>My daughter</option><option>My son</option><option>My sibling</option><option>A relative or friend</option></select></label><label className="field"><span>Ministry</span><select value={selectedMinistry} onChange={(event) => { setSelectedMinistry(event.target.value); setSelectedOrganisation(''); }}><option value="">Select your ministry</option>{ministryNames.map((ministry) => <option key={ministry} value={ministry}>{ministry}</option>)}</select></label><label className="field"><span>Department / organisation</span><select value={selectedOrganisation} onChange={(event) => setSelectedOrganisation(event.target.value)} disabled={!selectedMinistry}><option value="">{selectedMinistry ? 'Select department or organisation' : 'Select a ministry first'}</option>{organisations.map((entry) => <option key={`${entry.type}-${entry.name}`} value={entry.name}>{entry.name} · {entry.type}</option>)}</select></label><label className="field"><span>Service / cadre</span><select defaultValue=""><option value="" disabled>Select your service</option><option>Central Secretariat Service (CSS)</option><option>CSSS</option><option>Delhi Government service</option><option>Other NCR public service</option></select></label><label className="field"><span>Current NCR base</span><select defaultValue="Delhi"><option>Delhi</option><option>Gurugram</option><option>Noida</option><option>Ghaziabad</option><option>Faridabad</option></select></label><button className="button button--primary button--full" onClick={() => setStep(2)}>Next: your preferences <Icon name="arrow" size={16} /></button></>}
      {step === 2 && <><h2 id="onboard-title">What should your match know?</h2><p className="modal-copy">Share only what feels right. You control who sees every detail.</p><div className="choice-grid"><button className="choice-card choice-card--selected"><span>🏡</span><strong>Stay in NCR</strong><small>My priority is a shared NCR home</small></button><button className="choice-card"><span>↔</span><strong>Open to movement</strong><small>We can discuss future postings</small></button></div><label className="field"><span>Preferred NCR hubs</span><input defaultValue="Delhi, Noida, Gurugram" /></label><button className="button button--primary button--full" onClick={() => setStep(3)}>Next: verification <Icon name="arrow" size={16} /></button></>}
      {step === 3 && <><div className="success-orb"><Icon name="shield" size={28} /></div><h2 id="onboard-title">A trusted profile starts here</h2><p className="modal-copy">Verify your mobile today. Add service and identity documents when you’re ready; our team reviews them privately.</p><div className="verify-list"><div><span className="verify-number">1</span><span><strong>Mobile OTP</strong><small>Confirms you own the number</small></span><Icon name="check" size={16} /></div><div><span className="verify-number">2</span><span><strong>Service proof</strong><small>Visible only as a trust badge</small></span><Icon name="clock" size={16} /></div><div><span className="verify-number">3</span><span><strong>Photo + ID</strong><small>Encrypted and never public</small></span><Icon name="clock" size={16} /></div></div><button className="button button--primary button--full" onClick={onDone}>Open my dashboard <Icon name="arrow" size={16} /></button></>}
...
function Sidebar({ view, setView, onAdmin }: { view: View; setView: (v: View) => void; onAdmin: () => void }) {
  return <aside className="sidebar"><Logo /><div className="sidebar-label">Member home</div><div className="sidebar-nav">{navItems.map((item) => <button key={item.id} className={`side-link ${view === item.id ? 'side-link--active' : ''}`} onClick={() => setView(item.id)}><Icon name={item.icon} size={18} /><span>{item.label}</span>{item.id === 'inbox' && <i className="unread-dot" />}</button>)}</div><div className="sidebar-bottom"><button className="side-link side-link--admin" onClick={onAdmin}><Icon name="shield" size={18} /><span>Admin review</span><span className="admin-count">4</span></button><div className="sidebar-user"><Avatar initials="PS" tone="navy" /><span><strong>Priya S.</strong><small>Profile 72% complete</small></span><Icon name="chevron" size={15} /></div></div></aside>;
}

function Topbar({ view, onLogout }: { view: View; onLogout: () => void }) {
  const title = view === 'home' ? 'Good evening, Priya' : view === 'discover' ? 'Discover your aligned circle' : view === 'profile' ? 'My profile' : view === 'inbox' ? 'Your conversations' : view === 'settings' ? 'Privacy & safety' : 'Admin review';
...
function DiscoverView({ onOpenProfile, hiddenFromSearch, searchWhileHidden }: { onOpenProfile: () => void; hiddenFromSearch: boolean; searchWhileHidden: boolean }) {
  ...
  const filters = ['All aligned', 'Stay in NCR', 'CSS / CSSS', 'Delhi Govt', 'New this week'];
  const filtered = useMemo(() => {
    const searchableMatches = hiddenFromSearch && searchWhileHidden ? matches.filter((match) => match.gender === 'male') : matches;
    return active === 'CSS / CSSS' ? searchableMatches.filter((m) => m.role.includes('CSS')) : active === 'Delhi Govt' ? searchableMatches.filter((m) => m.role.includes('Delhi')) : active === 'New this week' ? searchableMatches.slice(1) : searchableMatches;
...
function ProfileView({ visibility }: { visibility: VisibilitySettings }) {
  const [editing, setEditing] = useState(false);
  return <div className="view-body profile-view"><div className="profile-cover"><div className="profile-cover-pattern" /><div className="profile-cover-content"><Avatar initials="PS" tone="navy" large /><div><span className="profile-id">NCR-04821 · <VerifiedBadge label="Mobile verified" /></span><h2>Priya Sharma</h2><p>Senior Assistant · Central Secretariat Service</p><p className="match-city"><span className="city-pin">⌖</span> New Delhi · Open to NCR</p></div><button className="button button--light button--small" onClick={() => setEditing(!editing)}>{editing ? 'Done editing' : 'Edit profile'} <Icon name="arrow" size={14} /></button></div></div><div className="profile-grid"><div className="profile-main"><div className="profile-card"><div className="card-heading"><div><span className="eyebrow">Your story</span><h3>A little more than a designation</h3></div><span className="edit-label">{editing ? 'Editing' : 'Visible to matches'}</span></div>{editing ? <textarea className="profile-textarea" defaultValue="I like early morning walks, old Hindi films, and a home where both people can pursue meaningful work. Looking for someone who sees a shared NCR life as an opportunity, not a compromise." /> : <p className="profile-story">I like early morning walks, old Hindi films, and a home where both people can pursue meaningful work. Looking for someone who sees a shared NCR life as an opportunity, not a compromise.</p>}<div className="story-tags"><span>Family-minded</span><span>Curious</span><span>Weekend explorer</span></div></div><div className="profile-card"><div className="card-heading"><div><span className="eyebrow">Career compatibility</span><h3>What you want your match to know</h3></div><span className="signal-badge"><Icon name="spark" size={13} /> Strong signal</span></div><div className="career-rows"><div><span>Preferred geography</span><strong>Delhi NCR</strong></div><div><span>Posting outlook</span><strong>Prefer stability in NCR</strong></div><div><span>Service</span><strong>CSS · Group B</strong></div>{visibility.ministry && <div><span>Ministry</span><strong>Ministry of Personnel, Public Grievances &amp; Pensions</strong></div>}{visibility.organisation && <div><span>Department / organisation</span><strong>Department of Personnel &amp; Training (DoPT)</strong></div>}<div><span>Work rhythm</span><strong>Mostly weekdays</strong></div></div></div></div><aside className="profile-side"><div className="profile-card completion-card--side"><div className="completion-head"><span>Profile strength</span><strong>72%</strong></div><div className="progress-track"><span style={{ width: '72%' }} /></div><ul className="profile-todos"><li className="done"><Icon name="check" size={14} /> Mobile verified</li><li className="done"><Icon name="check" size={14} /> NCR base added</li><li><Icon name="clock" size={14} /> Add service proof</li><li><Icon name="clock" size={14} /> Add a profile photo</li></ul></div><div className="profile-card trust-card"><Icon name="shield" size={20} /><h3>Trust centre</h3><p>Verification details stay private. Only the badge is visible on your profile.</p><button className="button button--text button--small">Open trust centre <Icon name="arrow" size={14} /></button></div></aside></div></div>;
}

function InboxView() {
  const [selected, setSelected] = useState(0);
  const conversations = [{ initials: 'AS', tone: 'rose', name: 'Aditi S.', role: 'Section Officer · CSS', text: 'I also think a shared NCR base…', time: '10:42 AM', unread: true }, { initials: 'NM', tone: 'sand', name: 'Nisha M.', role: 'Steno Grade B · CSSS', text: 'Your note about old films made me smile.', time: 'Yesterday', unread: false }, { initials: 'RK', tone: 'sage', name: 'Rohan K.', role: 'Assistant · Delhi Govt.', text: 'Sent you an interest', time: 'Tue', unread: false }];
...
export default function Page() {
  ...
  useEffect(() => { if (typeof window !== 'undefined' && window.localStorage.getItem('ncr-demo-mode') === 'member') setMode('app'); }, []);
  const enterApp = () => { window.localStorage.setItem('ncr-demo-mode', 'member'); setAuthOpen(false); setOnboardOpen(false); setMode('app'); };
  if (mode === 'app') return <AppShell onLogout={() => { window.localStorage.removeItem('ncr-demo-mode'); setMode('landing'); }} />;
  return <><Landing onAuth={() => setAuthOpen(true)} onOnboard={() => setOnboardOpen(true)} onDemo={enterApp} />{authOpen && <AuthModal onClose={() => setAuthOpen(false)} onEnter={enterApp} />}{onboardOpen && <OnboardModal onClose={() => setOnboardOpen(false)} onDone={enterApp} />}</>;
}
