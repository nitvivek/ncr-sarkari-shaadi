...
function AdminView({ reports, onSetStatus }: { reports: ReportItem[]; onSetStatus: (id: string, status: string) => void }) {
  const [tab, setTab] = useState<'reports' | 'review'>('reports');
  const queue = [{ initials: 'RM', tone: 'rose', name: 'Rahul M.', role: 'Assistant Section Officer · CSS', location: 'Noida · NCR', status: 'Service proof uploaded', time: '12 min ago' }, { initials: 'SK', tone: 'sage', name: 'Shreya K.', role: 'Stenographer Grade C · CSSS', location: 'Gurugram · NCR', status: 'Photo + ID uploaded', time: '38 min ago' }];
  const openReports = reports.filter((r) => r.status === 'open').length;
  return <div className="view-body admin-view"><div className="admin-top"><div><div className="eyebrow">Trust operations</div><h2>Keep the circle considered.</h2><p>Respond to member reports and keep every profile worthy of trust.</p></div></div><div className="admin-stats"><div><span>Open reports</span><strong>{openReports}</strong><small>Awaiting review</small></div><div><span>Total reports</span><strong>{reports.length}</strong><small>All time</small></div><div><span>Verification queue</span><strong>{queue.length}</strong><small>Demo — awaits R2 upload</small></div><div><span>Review model</span><strong>Manual</strong><small>Human-checked</small></div></div><div className="admin-tabs">{([['reports', 'Reports'], ['review', 'Verification queue']] as [typeof tab, string][]).map(([id, label]) => <button key={id} className={tab === id ? 'admin-tab--active' : ''} onClick={() => setTab(id)}>{label}{id === 'reports' && openReports > 0 && <span>{openReports}</span>}</button>)}</div>{tab === 'reports' && (reports.length === 0 ? <div className="empty-admin"><span><Icon name="shield" size={28} /></span><h3>No reports in the queue</h3><p>When a member reports a profile, it appears here for confidential review. Reporter identity is never shown to the reported member.</p></div> : <div className="review-table"><div className="review-table-head"><span>Reported member</span><span>Reason</span><span>From</span><span>Status</span></div>{reports.map((r) => <div className="review-row" key={r.id}><div><Avatar initials={initialsOf(r.reported_name)} tone="rose" /><span><strong>{maskLast(r.reported_name)}</strong><em>{r.detail || 'No detail provided'}</em></span></div><span className="document-status"><Icon name="shield" size={15} />{r.reason.replace(/_/g, ' ')}</span><em style={{ fontStyle: 'normal', fontSize: 11, color: 'var(--ink-muted)' }}>{maskLast(r.reporter_name)}</em><div className="review-actions"><select className="report-status-select" value={r.status} onChange={(e) => onSetStatus(r.id, e.target.value)}>{['open', 'reviewed', 'actioned', 'dismissed'].map((s) => <option key={s} value={s}>{s}</option>)}</select></div></div>)}</div>)}{tab === 'review' && <div className="review-table"><div className="review-table-head"><span>Member</span><span>Documents</span><span>Submitted</span><span>Action</span></div>{queue.map((member) => <div className="review-row" key={member.name}><div><Avatar initials={member.initials} tone={member.tone} /><span><strong>{member.name}</strong><small>{member.role}</small><em>{member.location}</em></span></div><span className="document-status"><Icon name="shield" size={15} />{member.status}</span><time>{member.time}</time><div className="review-actions"><button className="button button--outline button--small">Review</button></div></div>)}<div className="review-demo-note"><Icon name="spark" size={13} /> Verification queue is placeholder data — real document upload arrives with R2 (F19).</div></div>}</div>;
}

// ---- Real member-app data layer (wired to F13–F17 APIs) ----
type Member = { id: string; name: string; service: string | null; city: string | null; gender: string | null; postingOutlook: string | null; verified: boolean; photoMode: string };
...
function MemberModal({ member, status, onClose, onSend, onBlock, onReport }: { member: Member | null; status?: string; onClose: () => void; onSend: (id: string) => void; onBlock: (id: string) => void; onReport: (id: string, reason: string) => void }) {
  ...
  const label = status === 'accepted' ? 'Matched' : status === 'pending' ? 'Interest sent' : 'Send interest';
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal quick-profile" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}><button className="icon-button modal-close" aria-label="Close" onClick={onClose}><Icon name="x" /></button><div className="quick-profile-head"><Avatar initials={initialsOf(member.name)} tone="rose" large /><div>{member.verified && <VerifiedBadge />}<h2>{member.name}</h2><p>{member.service ?? 'Government professional'}</p><span className="match-city"><span className="city-pin"><Icon name="pin" size={13} /></span> {member.city ?? 'Delhi NCR'} · NCR</span></div></div><div className="quick-signal"><Icon name="lock" size={16} /><span><strong>Private by default</strong><small>{member.photoMode === 'verified' ? 'Photo visible to verified members.' : 'Photo on request — they approve each viewer.'}</small></span></div><p className="profile-story">Contact details and photographs stay private until this member chooses to share them.</p>{member.postingOutlook && <div className="quick-tags"><span>{member.postingOutlook}</span>{member.verified && <span>Verified</span>}</div>}{reporting ? <div className="report-box"><label className="field"><span>Why are you reporting this member?</span><select value={reason} onChange={(e) => setReason(e.target.value)}>{reportReasons.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}</select></label><div className="quick-actions"><button className="button button--primary button--full" onClick={() => onReport(member.id, reason)}>Submit report</button><button className="button button--outline button--full" onClick={() => setReporting(false)}>Cancel</button></div></div> : <><div className="quick-actions"><button className="button button--primary button--full" disabled={status === 'pending' || status === 'accepted'} onClick={() => onSend(member.id)}>{label}{!status && <Icon name="heart" size={16} />}</button><button className="button button--outline button--full" onClick={onClose}>Maybe later</button></div><div className="quick-secondary"><button className="text-button" onClick={() => onBlock(member.id)}>Block</button><span aria-hidden>·</span><button className="text-button" onClick={() => setReporting(true)}>Report</button></div></>}<p className="microcopy"><Icon name="lock" size={12} /> Contact details remain hidden until you both choose to share.</p></div></div>;
}
...
function AppShell({ user, onLogout }: { user: AuthUser | null; onLogout: () => void }) {
  ...
  useEffect(() => { if (user) refreshInterests(); }, [user]);
  useEffect(() => {
    if (!user) return;
  ...
  useEffect(() => {
    if (user?.role === 'admin' && view === 'admin') fetch('/api/report').then((r) => r.json()).then((d) => setReports(d.reports ?? [])).catch(() => undefined);
  }, [user, view]);
