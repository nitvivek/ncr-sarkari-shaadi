...
function RealDiscoverView({ members, statusFor, gender, setGender, onOpen, onSend }: { members: Member[]; statusFor: (id: string) => string | undefined; gender: string; setGender: (g: string) => void; onOpen: (m: Member) => void; onSend: (id: string) => void }) {
  ...
  return <div className="view-body"><div className="discover-toolbar"><div className="filter-scroll">{filters.map(([g, label]) => <button key={g} className={`filter-pill ${gender === g ? 'filter-pill--active' : ''}`} onClick={() => setGender(g)}>{label}</button>)}</div></div><p className="discover-hint">Showing members whose orientation matches yours. You can override above.</p><div className="discover-intro"><div><h2>{members.length} verified {members.length === 1 ? 'member' : 'members'}</h2><p>Everyone here is a government professional building a life in the Delhi NCR.</p></div></div>{members.length === 0 ? <div className="empty-admin"><span><Icon name="users" size={28} /></span><h3>No matches in this view</h3><p>This could be the community is still small — or no one's stated orientation matches yours yet. Try the other pills, or set your <em>Looking for</em> in your profile.</p></div> : <div className="match-grid match-grid--four">{members.map((m, i) => <RealMemberCard key={m.id} member={m} idx={i} status={statusFor(m.id)} onOpen={onOpen} onSend={onSend} />)}</div>}<div className="discover-footer"><Icon name="lock" size={16} /> Photos and contact details stay private until each member chooses to share them.</div></div>;
}
