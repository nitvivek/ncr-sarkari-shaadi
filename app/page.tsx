...
function HomeView({ setView, onOpenProfile, members, onOpenMember, onSend, statusFor }: { setView: (v: View) => void; onOpenProfile: () => void; members?: Member[]; onOpenMember?: (m: Member) => void; onSend?: (id: string) => void; statusFor?: (id: string) => string | undefined }) {
  const real = members && members.length > 0;
  return <div className="view-body view-body--home"><div className="welcome-banner"><div><span className="banner-kicker"><Icon name="spark" size={14} /> Your NCR circle</span><h2>{real ? 'Government professionals near you.' : 'People who understand the NCR life.'}</h2><p>{real ? 'Verified members building a life in the Delhi NCR.' : 'Based on your preference for a stable NCR posting and a thoughtful pace.'}</p><button className="button button--dark button--small" onClick={() => setView('discover')}>{real ? 'Browse everyone' : 'See all recommendations'} <Icon name="arrow" size={14} /></button></div><div className="banner-orbit"><div className="orbit orbit--one"><Avatar initials="AS" tone="rose" /></div><div className="orbit orbit--two"><Avatar initials="NM" tone="sand" /></div><div className="orbit orbit--three"><Avatar initials="RK" tone="sage" /></div><div className="orbit-core"><Icon name="heart" size={25} /></div></div></div><div className="section-heading-row"><div><h2>Good-fit profiles</h2><p>Career and life signals that line up with yours.</p></div><button className="button button--text button--small" onClick={() => setView('discover')}>View all <Icon name="arrow" size={14} /></button></div><div className="match-grid">{real ? members!.slice(0, 3).map((m, i) => <RealMemberCard key={m.id} member={m} idx={i} status={statusFor?.(m.id)} onOpen={onOpenMember!} onSend={onSend!} />) : matches.slice(0, 3).map((m) => <MatchCard key={m.id} match={m} onOpen={onOpenProfile} />)}</div><div className="home-lower-grid"><div className="completion-card"><div className="completion-head"><span><Icon name="user" size={17} /> Your profile</span><strong>72%</strong></div><div className="progress-track"><span style={{ width: '72%' }} /></div><p>Add your preferred NCR hubs and one line about your life outside work to improve your matches.</p><button className="button button--text button--small" onClick={() => setView('profile')}>Complete profile <Icon name="arrow" size={14} /></button></div><div className="safety-mini"><span className="safety-mini-icon"><Icon name="shield" size={18} /></span><div><strong>Safety check-in</strong><p>Your contact details are hidden from everyone until you accept an interest.</p><button className="button button--text button--small" onClick={() => setView('settings')}>Review privacy <Icon name="arrow" size={14} /></button></div></div></div></div>;
}
...
type Member = { id: string; name: string; service: string | null; city: string | null; gender: string | null; postingOutlook: string | null; verified: boolean; photoMode: string; hasPhoto?: boolean };
...
function RealMemberCard({ member, idx, status, onOpen, onSend }: { member: Member; idx: number; status?: string; onOpen: (m: Member) => void; onSend: (id: string) => void }) {
  ...
  return <article className="match-card"><div className="match-card-head">{showPhoto ? <img className="member-photo" src={`/api/photo?owner=${member.id}`} alt={`${member.name}'s photo`} loading="lazy" /> : <Avatar initials={initialsOf(member.name)} tone={tone} large />}</div><div className="match-card-body"><div className="match-name-row"><h3>{member.name}</h3>{member.verified && <VerifiedBadge />}</div><p className="match-role">{member.service ?? 'Government professional'}</p><p className="match-city"><span className="city-pin"><Icon name="pin" size={13} /></span>{member.city ?? 'Delhi NCR'} · NCR</p><div className="match-actions"><button className="button button--outline button--small" onClick={() => onOpen(member)}>View</button><button className="button button--primary button--small" disabled={status === 'pending' || status === 'accepted'} onClick={() => onSend(member.id)}>{label}{!status && <Icon name="heart" size={14} />}</button></div></div></article>;
}

function RealDiscoverView({ members, statusFor, gender, setGender, onOpen, onSend }: { members: Member[]; statusFor: (id: string) => string | undefined; gender: string; setGender: (g: string) => void; onOpen: (m: Member) => void; onSend: (id: string) => void }) {
  const filters: [string, string][] = [['all', 'Everyone'], ['female', 'Women'], ['male', 'Men']];
  return <div className="view-body"><div className="discover-toolbar"><div className="filter-scroll">{filters.map(([g, label]) => <button key={g} className={`filter-pill ${gender === g ? 'filter-pill--active' : ''}`} onClick={() => setGender(g)}>{label}</button>)}</div></div><div className="discover-intro"><div><h2>{members.length} verified {members.length === 1 ? 'member' : 'members'}</h2><p>Everyone here is a government professional building a life in the Delhi NCR.</p></div></div>{members.length === 0 ? <div className="empty-admin"><span><Icon name="users" size={28} /></span><h3>No one to show yet</h3><p>As verified members join, they’ll appear here. The community is just getting started.</p></div> : <div className="match-grid match-grid--four">{members.map((m, i) => <RealMemberCard key={m.id} member={m} idx={i} status={statusFor(m.id)} onOpen={onOpen} onSend={onSend} />)}</div>}<div className="discover-footer"><Icon name="lock" size={16} /> Photos and contact details stay private until each member chooses to share them.</div></div>;
}

type PhotoRequest = { id: string; status: string; user_id: string; full_name: string };

function InterestsInbox({ received, sent, photoRequests, onRespond, onPhotoRespond }: { received: InterestItem[]; sent: InterestItem[]; photoRequests: PhotoRequest[]; onRespond: (id: string, action: string) => void; onPhotoRespond: (id: string, action: string) => void }) {
...
function AppShell({ user, onLogout }: { user: AuthUser | null; onLogout: () => void }) {
  ...
  return <div className="app-shell"><Sidebar view={view} setView={changeView} onAdmin={() => changeView('admin')} displayName={displayName} initials={initials} showAdmin={showAdmin} /><div className="app-main"><Topbar view={view} onLogout={onLogout} firstName={firstNameOf(displayName)} initials={initials} />{view === 'home' && <HomeView setView={changeView} onOpenProfile={openDemoModal} members={isReal ? members : undefined} onOpenMember={openMember} onSend={sendInterest} statusFor={statusFor} />}{view === 'discover' && (isReal ? <RealDiscoverView members={members} statusFor={statusFor} gender={gender} setGender={setGender} onOpen={openMember} onSend={sendInterest} /> : <DiscoverView onOpenProfile={openDemoModal} hiddenFromSearch={visibility.hiddenFromSearch} searchWhileHidden={visibility.searchWhileHidden} />)}{view === 'profile' && <ProfileView visibility={visibility} user={user} profile={profile} onProfileSaved={setProfile} />}{view === 'inbox' && (isReal ? <InterestsInbox received={receivedInterests} sent={sentInterests} photoRequests={photoRequests} onRespond={respondInterest} onPhotoRespond={respondPhoto} /> : <InboxView />)}{view === 'settings' && <SettingsView settings={visibility} onToggle={toggleVisibility} />}{view === 'admin' && <AdminView reports={reports} onSetStatus={setReportStatus} pendingVerifications={pendingVerifications} onVerifyDecision={decideVerification} />}</div>{profileOpen && <MemberModal member={selected} status={selected ? statusFor(selected.id) : undefined} onClose={() => { setProfileOpen(false); setSelected(null); }} onSend={sendInterest} onBlock={blockMember} onReport={reportMember} />}{toast && <div className="app-toast" role="status">{toast}</div>}</div>;
}
