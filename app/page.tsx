...
function MemberModal({ member, status, onClose, onSend, onBlock, onReport }: { member: Member | null; status?: string; onClose: () => void; onSend: (id: string) => void; onBlock: (id: string) => void; onReport: (id: string, reason: string) => void }) {
  ...
  const photoLabel = photoStatus === 'open' ? 'Visible to verified members' : photoStatus === 'granted' ? 'Access granted — image upload arrives with verification' : photoStatus === 'requested' ? 'Awaiting their approval' : photoStatus === 'hidden' ? 'Kept private by this member' : 'Ask to view — they approve each request';
