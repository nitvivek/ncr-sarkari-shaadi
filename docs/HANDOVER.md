...
- **Phase B (backend, later):** add columns/JSON to `profiles` (keep single-table pattern, NOT the 10-table source schema) + `partner_preferences` + `photos` (multi) + horoscope image (R2); extend `/api/profile` + `/api/preferences`; enforce set-once immutables server-side; migrate localStorage draft → server; wire matching/search (sensitive fields excluded from filters).
