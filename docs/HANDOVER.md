...
| Orientation-aware discovery | this session | `/api/members` rewritten to compute the viewer's target gender from `profiles.gender` + `partner_preferences.pref_looking_for`, then enforce (a) target gender filter, (b) other-side-agrees filter (the viewed profile's `pref_looking_for` must include the viewer's gender, or be `Any / All Genders`), (c) verified-only, plus all existing exclusions (self, hidden, blocks both ways). `?gender=male\|female\|all` is now an explicit override; absent → default to viewer's own pref. AppShell auto-defaults the Discover gender pill from `profile.gender`. New "Verified identity" card in Settings shows gender + pref_looking_for with the ID-verification note + contact-support link. End-to-end curl-verified: Priya (F, pref=Male) sees 3 males, Aarav (M, pref=Female) sees 3 females, self-excluded, blocks honored, non-default orientations respected (no fake results). No schema changes; no admin changes. |
| Sign-out + state reset | this session | Added explicit "Sign out" button in the topbar (topbar-avatar still clickable). Hardened the logout() handler: clears `ncr-profile-draft` localStorage key, unregisters the service worker and clears its caches, then full-reloads the page so no in-flight fetches leak the previous user's data into the next session. Fixes the bug where member-list / photo caches from one account could bleed into Discover after a re-login. |
